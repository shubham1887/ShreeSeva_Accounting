...
public class CreditNoteRepository : ICreditNoteRepository
{
    ...
    public CreditNoteRepository(IDbConnectionFactory db, ISettingsRepository settings)
    {
        _db = db;
        _settings = settings;
    }

    public async Task<int> CreateAsync(CreditNoteMaster master, List<CreditNoteDetail> details)
    {
            ...
            foreach (var d in details)
            {
                ...
                dc.CommandText = @"
                    INSERT INTO CreditNoteDetails
                        (TenantId, CreditNoteMasterId, ProductId, BatchNo, ExpiryMY, ReturnQty,
                         SaleRate, MRP, SGSTRate, CGSTRate, IGSTRate, SGSTAmount, CGSTAmount, IGSTAmount,
                         TaxableAmount, LineTotal, IsDeleted, CreatedAt)
                    VALUES
                        (@tid, @cnid, @pid, @batch, @exp, @qty,
                         @rate, @mrp, @sgst, @cgst, @igst, @sgsta, @cgsta, @igsta,
                         @taxable, @lt, 0, @now)";
                dc.Parameters.AddWithValue("@tid", _db.TenantId);
                dc.Parameters.AddWithValue("@cnid", masterId);
    ...
    public async Task<CreditNoteMaster?> GetByIdAsync(int id)
    {
        using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var cmd = conn.CreateCommand();
        cmd.CommandText = @"
            SELECT cn.*, a.AccountName
            FROM CreditNoteMaster cn
            LEFT JOIN Accounts a ON cn.AccountId = a.Id
            WHERE cn.Id = @id AND cn.TenantId = @tid AND cn.IsDeleted = 0";
    ...
    public async Task<List<CreditNoteMaster>> GetByDateRangeAsync(DateTime from, DateTime to, int? accountId = null)
    {
        using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var cmd = conn.CreateCommand();
        cmd.CommandText = @"
            ...
            FROM CreditNoteMaster cn
            LEFT JOIN Accounts a ON cn.AccountId = a.Id
            WHERE cn.TenantId = @tid AND cn.IsDeleted = 0
              AND cn.VoucherDate BETWEEN @from AND @to";
        if (accountId.HasValue)
        {
            cmd.CommandText += " AND cn.AccountId = @aid";
            cmd.Parameters.AddWithValue("@aid", accountId.Value);
        }
        cmd.CommandText += " ORDER BY cn.VoucherDate DESC, cn.Id DESC";
        cmd.Parameters.AddWithValue("@tid", _db.TenantId);
        cmd.Parameters.AddWithValue("@from", from.ToString("yyyy-MM-dd"));
        cmd.Parameters.AddWithValue("@to", to.ToString("yyyy-MM-dd"));
        var list = new List<CreditNoteMaster>();
        using var reader = await cmd.ExecuteReaderAsync();
    ...
    public async Task<List<CreditNoteDetail>> GetDetailsAsync(int masterId)
    {
        using var conn = _db.CreateConnection();
        await conn.OpenAsync();
        var cmd = conn.CreateCommand();
        cmd.CommandText = @"
            ...
            FROM CreditNoteDetails d
            LEFT JOIN Products p ON d.ProductId = p.Id
            WHERE d.CreditNoteMasterId = @mid AND d.TenantId = @tid AND d.IsDeleted = 0";
        cmd.Parameters.AddWithValue("@mid", masterId);
        cmd.Parameters.AddWithValue("@tid", _db.TenantId);
        var list = new List<CreditNoteDetail>();
        using var reader = await cmd.ExecuteReaderAsync();
        while (await reader.ReadAsync())
            list.Add(new CreditNoteDetail
            {
                Id = reader.GetInt32(reader.GetOrdinal("Id")),
                CreditNoteMasterId = reader.GetInt32(reader.GetOrdinal("CreditNoteMasterId")),
                ...
                SaleRate = reader.GetDecimal(reader.GetOrdinal("SaleRate")),
    ...
    public Task<string> GetNextVoucherNoAsync(string financialYear)
        => _settings.GetNextVoucherNoAsync("CN", financialYear);

    private static CreditNoteMaster MapMaster(SqliteDataReader r) => new()
    {
